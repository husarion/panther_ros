...
class panther_gazebo::PantherSystemPrivate
{
public:
  PantherSystemPrivate() = default;
...
{
void PantherSystem::readParams(const hardware_interface::HardwareInfo & hardware_info)
{
  if (
    hardware_info.hardware_parameters.find("e_stop_initial_state") !=
    hardware_info.hardware_parameters.end()) {
    std::istringstream(hardware_info.hardware_parameters.at("e_stop_initial_state")) >>
      std::boolalpha >> this->dataPtr->e_stop_active;
    RCLCPP_INFO(
      this->nh_->get_logger(), "Parameter 'e_stop_initial_state' found, using: %s",
      this->dataPtr->e_stop_active ? "true" : "false");
  } else {
...
{
  this->dataPtr->e_stop_publisher = nh_->create_publisher<std_msgs::msg::Bool>(
    "~/e_stop", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());

  this->dataPtr->e_stop_reset_service = nh_->create_service<std_srvs::srv::Trigger>(
    "~/e_stop_reset",
    std::bind(
      &PantherSystem::eStopResetCallback, this, std::placeholders::_1, std::placeholders::_2));

  this->dataPtr->e_stop_trigger_service = nh_->create_service<std_srvs::srv::Trigger>(
    "~/e_stop_trigger",
    std::bind(
...
void PantherSystem::publishEStopStatus()
{
  ...
  e_stop_msg.data = this->dataPtr->e_stop_active;
  this->dataPtr->e_stop_publisher->publish(e_stop_msg);
}
...
{
  this->dataPtr = std::make_unique<PantherSystemPrivate>();
  ...
  for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
    ...
    if (joint_info.parameters.find("mimic") != joint_info.parameters.end()) {
      ...
      RCLCPP_INFO_STREAM(
        this->nh_->get_logger(), "Joint '" << joint_name << "'is mimicking joint '"
                                           << mimicked_joint
  ...
  readParams(hardware_info);
  setupEStop();

  return true;
}

void PantherSystem::registerSensors(const hardware_interface::HardwareInfo & hardware_info)
{
...
std::vector<hardware_interface::StateInterface> PantherSystem::export_state_interfaces()
{
  return std::move(this->dataPtr->state_interfaces_);
}

std::vector<hardware_interface::CommandInterface> PantherSystem::export_command_interfaces()
{
...
hardware_interface::return_type PantherSystem::read(
...
hardware_interface::return_type PantherSystem::perform_command_mode_switch(
  ...
  for (unsigned int j = 0; j < this->dataPtr->joints_.size(); j++) {
    for (const std::string & interface_name : stop_interfaces) {
      ...
      if (
        ...
        interface_name == (this->dataPtr->joints_[j].name + "/" +  // NOLINT
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  if (this->dataPtr->e_stop_active) {
    for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i) {
      ...
      if (vel == nullptr) {
        this->dataPtr->ecm->CreateComponent(
          this->dataPtr->joints_[i].sim_joint, ignition::gazebo::components::JointVelocityCmd({0}));
  ...
  for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i) {
    ...
    if (this->dataPtr->joints_[i].joint_control_method & VELOCITY) {
      ...
      double error;
      error =
        (this->dataPtr->joints_[i].joint_position - this->dataPtr->joints_[i].joint_position_cmd) *
      ...
      if (!this->dataPtr->ecm->Component<ignition::gazebo::components::JointForceCmd>(
            this->dataPtr->joints_[i].sim_joint)) {
        this->dataPtr->ecm->CreateComponent(
          this->dataPtr->joints_[i].sim_joint, ignition::gazebo::components::JointForceCmd({0}));
      } else {
  ...
  for (const auto & mimic_joint : this->dataPtr->mimic_joints_) {
    for (const auto & mimic_interface : mimic_joint.interfaces_to_mimic) {
      if (mimic_interface == "position") {
        ...
        double position_error = position_mimic_joint -
