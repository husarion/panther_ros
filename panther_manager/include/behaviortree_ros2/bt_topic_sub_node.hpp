...
template <class T>
inline RosTopicSubNode<T>::RosTopicSubNode(
...
inline bool RosTopicSubNode<T>::createSubscriber(const std::string & topic_name)
{
  ...
  if (it == registry.end()) {
    ...
    RCLCPP_INFO(
      logger(), "Node [%s] created Subscriber to topic [%s]", name().c_str(), topic_name.c_str());
  } else {
    sub_instance_ = it->second;
  }
  ...
  signal_connection_ =
    sub_instance_->broadcaster.connect([this](const std::shared_ptr<T> msg) { last_msg_ = msg; });
