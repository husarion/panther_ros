...
protected:
  ...
  panther_hardware_interfaces::RoboteqDriverInterface::SharedPtr roboteq_driver_;
};
...
TEST_F(TestRoboteqDriver, ReadRoboteqMotorStates)
{
  ...
  const std::int32_t left_pos = 101;
  const std::int32_t left_vel = 102;
  const std::int32_t left_current = 103;
  const std::int32_t right_pos = 201;
  const std::int32_t right_vel = 202;
  const std::int32_t right_current = 203;

  roboteqs_mock_->GetDriver()->SetPosition(DriverChannel::CHANNEL2, left_pos);
  roboteqs_mock_->GetDriver()->SetPosition(DriverChannel::CHANNEL1, right_pos);

  roboteqs_mock_->GetDriver()->SetVelocity(DriverChannel::CHANNEL2, left_vel);
  roboteqs_mock_->GetDriver()->SetVelocity(DriverChannel::CHANNEL1, right_vel);

  roboteqs_mock_->GetDriver()->SetCurrent(DriverChannel::CHANNEL2, left_current);
  roboteqs_mock_->GetDriver()->SetCurrent(DriverChannel::CHANNEL1, right_current);

  std::this_thread::sleep_for(std::chrono::milliseconds(50));

  panther_hardware_interfaces::RoboteqMotorsStates fb =
    roboteq_driver_->ReadRoboteqMotorsStates();

  EXPECT_EQ(fb.motor_2.pos, left_pos);
  EXPECT_EQ(fb.motor_2.vel, left_vel);
  EXPECT_EQ(fb.motor_2.current, left_current);

  EXPECT_EQ(fb.motor_1.pos, right_pos);
  EXPECT_EQ(fb.motor_1.vel, right_vel);
  EXPECT_EQ(fb.motor_1.current, right_current);
}
...
TEST_F(TestRoboteqDriver, ReadRoboteqMotorStatesTimestamps)
{
  ...
  panther_hardware_interfaces::RoboteqMotorsStates fb1 =
    roboteq_driver_->ReadRoboteqMotorsStates();

  // based on publishing frequency in the Roboteq mock (10)
  std::this_thread::sleep_for(std::chrono::milliseconds(10));

  panther_hardware_interfaces::RoboteqMotorsStates fb2 =
    roboteq_driver_->ReadRoboteqMotorsStates();
  ...
  EXPECT_LE(
    lely::util::from_timespec(fb2.pos_timestamp) - lely::util::from_timespec(fb1.pos_timestamp),
  ...
  EXPECT_LE(
    lely::util::from_timespec(fb2.vel_current_timestamp) -
      lely::util::from_timespec(fb1.vel_current_timestamp),
    std::chrono::milliseconds(15));
...
TEST_F(TestRoboteqDriver, ReadRoboteqDriverState)
{
  ...
  panther_hardware_interfaces::RoboteqDriverState fb =
    roboteq_driver_->ReadRoboteqDriverState();
...
TEST_F(TestRoboteqDriver, ReadRoboteqDriverStateTimestamp)
{
  ...
  panther_hardware_interfaces::RoboteqDriverState fb1 =
    roboteq_driver_->ReadRoboteqDriverState();

  // based on publishing frequency in the Roboteq mock (50)
  std::this_thread::sleep_for(std::chrono::milliseconds(50));

  panther_hardware_interfaces::RoboteqDriverState fb2 =
    roboteq_driver_->ReadRoboteqDriverState();
...
TEST_F(TestRoboteqDriver, SendRoboteqCmd)
{
  ...
  const std::int32_t left_v = 10;
  const std::int32_t right_v = 20;

  roboteq_driver_->SendRoboteqCmd(right_v, left_v);

  std::this_thread::sleep_for(std::chrono::milliseconds(50));

  EXPECT_EQ(roboteqs_mock_->GetDriver()->GetRoboteqCmd(DriverChannel::CHANNEL2), left_v);
  EXPECT_EQ(roboteqs_mock_->GetDriver()->GetRoboteqCmd(DriverChannel::CHANNEL1), right_v);
}
...
TEST_F(TestRoboteqDriver, ResetRoboteqScript)
{
  ...
  roboteq_driver_->ResetRoboteqScript();
...
TEST_F(TestRoboteqDriver, TurnOnSafetyStopChannel1)
{
  ...
  roboteq_driver_->TurnOnSafetyStopChannel1();
...
TEST_F(TestRoboteqDriver, TurnOnSafetyStopChannel2)
{
  ...
  roboteq_driver_->TurnOnSafetyStopChannel2();
